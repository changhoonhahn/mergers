...
def merger_track_throughout(mmid, dlogm=0.5): 
    ...
    M0 = sub[0]['m.max'][ihalos]
    m_M0 = []
    for isnap in range(snapshots[-1]): 
        # indices of the parent at snap = i+1 with the highest M_max (i.e. primaries) 
        i_primaries = sub[isnap]['par.i'][ihalos]
        ...
        ihalos = ihalos[has_primary]
        i_primaries = i_primaries[has_primary]
        print("%i primaries at %i" % (len(i_primaries), isnap))
        
        # make sure indices match across snapshot i and i+1 
        assert np.sum(ihalos - sub[isnap+1]['chi.i'][i_primaries]) == 0
        
        # identify halos at i+1 that merge into one children
        uniq_chi, n_chi = np.unique(sub[isnap+1]['chi.i'], return_counts=True)
        notonly_chi = (n_chi > 1) & (uniq_chi != -1) 
        # only keep halos that are primaries at i 
        ih_wmerger, ii, _ = np.intersect1d(ihalos, uniq_chi[notonly_chi], return_indices=True) 
        for ih, iii in zip(ih_wmerger, ii): 
            is_parent = np.where(sub[isnap+1]['chi.i'] == ih)[0]
            i_primary = sub[isnap]['par.i'][ih]
            notprimary = (is_parent != i_primary)
            m_M0_i = 10.**(sub[isnap+1]['m.max'][is_parent[notprimary]] - M0[iii])
            m_M0 += list(m_M0_i) # M_nonprimary / M0 
        ihalos = i_primaries

    print('%i halos have primaries in the last snapshot' % len(ihalos)) 
    m_M0 = np.array(m_M0)
    mp_M0 = np.array(mp_M0) 
    f = h5py.File('m_M0.h5', 'w') 
    f.create_dataset('m_M0', data=m_M0) 
    f.close() 
    return None 
...
if __name__=="__main__": 
    ...
    #merger_track_Mprimary(12, dlogm=0.5, nsnap_final=35)
    plotMzM0(35)
